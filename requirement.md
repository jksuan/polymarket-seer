...
- **比赛卡片 (MatchCard)**：
  - 顶部：联赛名称 + 比赛倒计时/实时比分
  - 中段：主队 VS 客队，带国旗/队徽、实时胜率百分比
  - 底部：三向投注按钮（买主胜 / 买平局 / 买客胜），显示赔率倍数
  - 扩展信息：用户持仓标签、交易量、人气指数
- **趣味投注卡片 (OutrightCard)**：长期预测市场（如"世界杯冠军"、"最佳射手"等），显示各选项的胜率和赔率
- **加载态**：`MatchCardSkeleton` / `OutrightCardSkeleton` — 与真实卡片 1:1 等高骨架

#### 3.1.3 交互流
1. 用户点击投注按钮 → 弹出确认下单弹窗 (ConfirmModal)
2. 弹窗内展示：选中方向、实时赔率、预期收益、滑动金额选择器
3. 确认下注 → TxOverlay 展示链上交易进度（签名中 → 提交中 → 等待确认 → 成功/失败）
...
| **缓存策略** | SWR 去重 + 模块级缓存，避免频繁切换页面时重复拉取 |
| **交易延迟** | 下单弹窗内使用高频轮询（3 秒）确保赔率准确 |
| **布局稳定** | 禁止 Layout Shift，所有加载态与真实态等高、等宽 |
