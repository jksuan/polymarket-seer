...
function StatBox({ label, value, valueColor, sub, subColor }: {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 13, color: '#8b94a8', marginBottom: 8, display: 'flex' }}>{label}</div>
      <div style={{ fontSize: 20, fontWeight: 700, color: valueColor, display: 'flex' }}>{value}</div>
      {sub && <div style={{ fontSize: 13, fontWeight: 700, color: subColor || '#fff', marginTop: 4, display: 'flex' }}>{sub}</div>}
    </div>
  );
}
    ...
    const statsContent = type === 'position' ? (
      <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 13, color: '#8b94a8', marginBottom: 8, display: 'flex' }}>胜率变化</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: '#a3aac4' }}>{avgPctStr}%</span>
            <span style={{ fontSize: 14, color: '#4FC3F7' }}>→</span>
            <span style={{ fontSize: 16, fontWeight: 700, color: '#dee5ff' }}>{curPctStr}%</span>
          </div>
        </div>
      </div>
    ) : (
      <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
        <div style={{
          ...
          <div style={{ fontSize: 13, color: '#8bcc5a', marginBottom: 8, display: 'flex' }}>净盈利</div>
          <div style={{ fontSize: 36, fontWeight: 900, color: '#ADFF2F', display: 'flex' }}>+${Number(usdcAmt).toFixed(2)}</div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 900, marginLeft: 10, display: 'flex', alignItems: 'center' }}>
            ...
            <div style={{
              ...
              fontSize: 16, fontWeight: 700, color: badgeColor,
            }}>
              {badgeText}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', marginTop: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', marginLeft: iconBase64 ? 16 : 0, flex: 1 }}>
              <div style={{ fontSize: 20, fontWeight: 600, color: '#dee5ff', lineHeight: 1.3, display: 'flex', maxHeight: 54, overflow: 'hidden' }}>
                {displayTitle}
              </div>
              {outcome && (
          ...
          <div style={{
            ...
            <div style={{ fontSize: 16, color: '#7a8398', fontWeight: 500, display: 'flex' }}>{footerText}</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: '#ADFF2F', display: 'flex' }}>seer.sports</div>
          </div>
        </div>
      ),
      {
