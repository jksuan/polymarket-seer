...
- **Withdraw**：从 Polymarket **Deposit Wallet（交易金库）** 经 Bridge 将 **Polygon pUSD** 提到用户指定的外部 EVM 地址；到账 token/chain **固定**为 Polygon · PUSD（只读展示）。用户确认后 `POST /withdraw` 生成临时 bridge 入金地址，relayer 以 Deposit Wallet batch 执行 pUSD `transfer` 至 bridge；Bridge `/status` 轮询至完成。
...
- **Trading Vault（Deposit Wallet）**：Privy EOA 为签名者；链上 funder / 持仓地址为 Polymarket **Deposit Wallet**（CLOB type 3，`POLY_1271`）。UI 设置页仍可称「智能金库」。
- **ensureProxyCollateralSynced**：`updateBalanceAllowance` → `getBalanceAllowance(COLLATERAL)`；若 CLOB=0 且金库仍有 legacy USDC.e，经 relayer 执行 **Collateral Onramp wrap** 后再 sync。
- **交易授权**：下注前 pUSD approve（含 Exchange V2 / Neg Risk）；卖出前 ERC1155 `setApprovalForAll`（见 ADR-0006）。
- **实现集中**：`src/auth/collateralBalance.ts`、`src/auth/vault/`、`readUsdcBalanceDisplay.ts`；细则见 `docs/adr/0004-polymarket-auth-invariants.md` §1 与 `docs/adr/0006-deposit-wallet-clob-v2.md`。
