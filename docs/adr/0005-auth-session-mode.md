...
- Provider 配置 `embeddedWallets.ethereum.createOnLogin: 'off'`（2026-05-30 修订：避免与内置开户屏竞态导致双钱包及 `createWallet` 崩溃）。
- `embedded` 会话若缺少与当前 user 匹配的 privy 钱包，由 `PolymarketAuthContext.ensureEmbeddedWalletForUser()` 显式 `createWallet()`（单路径；换账号时同样兜底）。

### 7. 继承 ADR-0004 的余额与 CLOB 不变量

以下条款 **继续有效**（余额口径见 ADR-0004 §1 2026-05-28 修订）：

- 顶栏余额：`ensureProxyCollateralSynced`（CLOB sync → 必要时 USDC.e→pUSD wrap → 再 sync）；展示 = CLOB 可交易 collateral（`collateralBalance.ts` / `readUsdcBalanceDisplay.ts`）。
...
## 修订（2026-05-20）— Telegram 弹窗入口可选

- Privy 弹窗默认 **不展示** `telegram`（`Providers.tsx` + `NEXT_PUBLIC_ENABLE_TELEGRAM_LOGIN`，默认 `false`）。
- `privyUserIdentity` 中 Telegram 身份解析 **保留**；设为 `true` 并配置 Privy Dashboard / BotFather 后可恢复弹窗入口。

## 修订（2026-05-30）— Embedded 钱包单路径创建

- **问题**：`createOnLogin: users-without-wallets` 与 `ensureEmbeddedWalletForUser()` 并行，新用户首登 Privy 后台出现两个 Embedded Wallet，且 Privy 内置 `EmbeddedWalletOnAccountCreateScreen` 与手动 `createWallet` 竞态崩溃。
- **决策**：`Providers.tsx` 设 `createOnLogin: off`；钱包创建 **仅** 由 `PolymarketAuthContext.ensureEmbeddedWalletForUser()` 负责；调用前校验 `createWallet` 可用且尚无 matching privy 钱包。
